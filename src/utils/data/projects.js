export const projects = [
  {
    name: "QuickSplit [w.i.p]",
    // emoji: "🛠️",
    techStack: "Python, FastAPI, Pydantic, JWT Auth, React, JavaScript, TypeScript, Ant Design",
    description:
      "A complete, full-stack, end-to-end expense splitting application/web app",
    githubLink: "https://github.com/NishantGada/quick-split-v2",
    show: true
  },
  {
    name: "Byte Blogs [w.i.p]",
    emoji: "🛠️",
    techStack: "Node.js, Express, JavaScript, TypeScript, React, NoSQL / AWS DynamoDB, Chakra UI, Vite",
    description:
      "A simple, second, personal blogging platform I am developing to further expand my skills, technical depth, use AI more efficiently, and this time hopefully actually write something.",
    githubLink: "https://github.com/NishantGada/byte-blogs",
    show: true
  },
  {
    name: "ComfortZone",
    emoji: "🛠️",
    techStack: "Python, Flask, MySQL",
    description:
      "A project that I think would be relevant to many. This is a toilet aggregator/searching app that allows the user to find a nearby toilet, rate it, add pictures, and further information. This improves accessibility, comfort, and hygiene for everyone.",
    githubLink: "https://github.com/NishantGada/comfort-zone-flask-backend",
    show: false
  },
  {
    name: "Jiraffe",
    emoji: "",
    techStack: "Java, Spring/Spring Boot, Hibernate, SQL",
    description:
      "Developed a bug tracking application with a user-friendly JSP interface, implementing role-based authorization to streamline bug reporting, assignment, and tracking. Leveraged Hibernate ORM and DAO patterns for efficient data management and real-time updates, and designed an advanced dashboard with filtering and sorting capabilities.",
    githubLink: "https://github.com/NishantGada/bug-tracker",
    show: true
  },
  {
    name: "BostonTown",
    emoji: "",
    techStack: "React, Node.js, Express, MongoDB, Ant Design",
    description:
      "Led a team of 4 to develop a Boston-based listing website using the MERN stack, featuring over 300,000 properties for improved apartment search. Designed a user reporting feature with an interactive heat map for increased trust and informed decision-making.",
    githubLink: "https://github.com/NishantGada/BostonTown",
    show: true
  },
  {
    name: "Unwritten",
    emoji: "",
    techStack: "Python, Django, JavaScript, CSS, Tailwind CSS, SQL",
    description:
      "Developed a full-stack blogging platform with a robust backend supporting blog creation, media uploads, and timestamp tracking. Designed advanced search and filtering features to improve navigation and content discovery.",
    githubLink: "https://github.com/NishantGada/unwritten",
    show: true
  }
];